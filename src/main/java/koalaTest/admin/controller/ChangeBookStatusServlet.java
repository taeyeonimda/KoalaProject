...
/**
 * Servlet implementation class ChangeBookStatusServlet
 */
@WebServlet(name = "ChangeBookStatus", urlPatterns = { "/changeBookStatus.do" })
public class ChangeBookStatusServlet extends HttpServlet {
     ...
     */
    public ChangeBookStatusServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String bookStatus = request.getParameter("selectedStatus");
		// 3. 비즈니스 로직
		BookService service = new BookService();
		int result = service.changeBookStatus(bookNo, bookStatus);
		// 4. 결과 처리
		RequestDispatcher view = request.getRequestDispatcher("/WEB-INF/views/common/msg.jsp");
		if(result > 0) {
			request.setAttribute("title", "상태 변경 성공");
			request.setAttribute("msg", "책 판매 상태가 변경되었습니다.");
